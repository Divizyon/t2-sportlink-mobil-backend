...
exports.userService = {
    ...
    async getProfile(userId) {
        try {
            ...
            const createdEventsCount = await prisma_1.default.event.count({
                where: { creator_id: userId }
            });
            // Katılınan etkinlik sayısını al
            const participatedEventsCount = await prisma_1.default.event_participant.count({
                where: { user_id: userId }
            });
            // Kullanıcının katıldığı aktif etkinlikleri al
            const participatedEvents = await prisma_1.default.event_participant.findMany({
                where: {
                    user_id: userId,
            ...
            const createdButNotParticipatedEvents = await prisma_1.default.event.findMany({
                where: {
                    creator_id: userId,
                    status: {
            ...
            const userRatings = await prisma_1.default.user_rating.findMany({
            ...
            // Arkadaş sayısını al
            const friendsCount = await prisma_1.default.friend.count({
                where: {
                    OR: [
                        { user_id1: userId },
                        { user_id2: userId }
    ...
    async updateProfile(userId, data) {
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...updatedUser,
                    sports: data.sportInterest ? (_a = sportResult === null || sportResult === void 0 ? void 0 : sportResult.data) === null || _a === void 0 ? void 0 : _a.sports : undefined
                }
            };
        }
        catch (error) {
    ...
    async updateProfilePicture(userId, file) {
        try {
            ...
            if (user === null || user === void 0 ? void 0 : user.profile_picture) {
                ...
                if (oldFilePath) {
                    await supabase_1.supabase.storage.from('profile-pictures').remove([oldFilePath]);
                }
            }
            ...
            // Supabase'e yükle
            const { error: uploadError, data } = await supabase_1.supabase.storage
     ...
     */
    async updateSports(userId, sports) {
        try {
            // Önce kullanıcının mevcut spor dallarını temizle
            await prisma_1.default.user_sport.deleteMany({
                where: { user_id: userId }
            });
            // Yeni spor dallarını ekle
            for (const sport of sports) {
                await prisma_1.default.user_sport.create({
                    data: {
                        user: {
                            connect: { id: userId }
                        },
            ...
            const updatedUserSports = await prisma_1.default.user_sport.findMany({
                where: { user_id: userId },
                include: { sport: true }
            });
            return {
     ...
     */
    async selectSportInterests(userId, sports) {
        try {
            ...
            const existingSports = await prisma_1.default.sport.findMany({
                where: {
                    id: {
                        in: sportIds
                    }
     ...
     */
    async addSportInterest(userId, sportData) {
        try {
            // Kullanıcının varlığını kontrol et
            const user = await prisma_1.default.user.findUnique({
                where: { id: userId }
            ...
            const sportExists = await prisma_1.default.sport.findUnique({
                where: {
            ...
            const existingUserSport = await prisma_1.default.user_sport.findUnique({
                where: {
                    user_id_sport_id: {
                        user_id: userId,
            ...
            if (existingUserSport) {
                ...
                await prisma_1.default.user_sport.update({
                    where: {
                        user_id_sport_id: {
                            user_id: userId,
                            sport_id: sportData.sportId
    ...
    async updateExpiredEvents() {
        try {
            ...
            const expiredEvents = await prisma_1.default.event.findMany({
                where: {
                    status: 'active',
                    end_time: {
                        lt: now // end_time < now
            ...
            const updateResult = await prisma_1.default.event.updateMany({
                where: {
                    id: {
