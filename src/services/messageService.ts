...
import { Conversation, ConversationParticipant, Message } from '../models';
...
export const messageService = {
   ...
   */
  async sendMessage(conversationId: string, senderId: string, content: string, mediaUrl?: string): Promise<Message> {
    // Kullanıcının bu konuşmanın bir katılımcısı olduğunu doğrula
    const participant = await prisma.conversation_participant.findUnique({
      where: {
   ...
   */
  async getConversationMessages(conversationId: string, userId: string, limit: number = 20, offset: number = 0) {
    // Kullanıcının bu konuşmanın bir katılımcısı olduğunu doğrula
    const participant = await prisma.conversation_participant.findUnique({
      where: {
        conversation_id_user_id: {
      ...
      if (participant.is_admin) {
        ...
        if (otherAdmins.length === 0) {
          const activeParticipants = conversation.participants.filter(
            p => p.left_at === null && p.user_id !== userId
          );
