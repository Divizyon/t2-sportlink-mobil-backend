...
import { calculateDistance, calculateBulkDistances } from '../utils/maps.util';
 ...
 */
export const getBulkDistances = async (req: Request, res: Response) => {
  try {
    ...
    const result = await calculateBulkDistances(
      origin,
      destinations,
      mode as 'driving' | 'walking' | 'bicycling' | 'transit'
    );
