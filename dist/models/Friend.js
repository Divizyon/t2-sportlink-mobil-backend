...
class Friend {
    ...
    static async sendRequest(senderId, receiverId) {
        ...
        const existingRequest = await prisma_1.default.friendship_request.findFirst({
            where: {
                OR: [
                    {
                        sender_id: senderId,
        ...
        // Yeni istek oluştur
        return prisma_1.default.friendship_request.create({
            data: {
    ...
    static async acceptRequest(requestId, userId) {
        ...
        const request = await prisma_1.default.friendship_request.findFirst({
            where: {
                id: requestId,
                receiver_id: userId,
                status: 'pending'
        ...
        const [updatedRequest, newFriendship] = await prisma_1.default.$transaction([
            // İstek durumunu güncelle
            prisma_1.default.friendship_request.update({
                where: { id: requestId },
    ...
    static async rejectRequest(requestId, userId) {
        ...
        // İsteği güncelle
        return prisma_1.default.friendship_request.update({
            where: { id: requestId },
    ...
    static async removeFriend(userId, friendId) {
        ...
        const friendship = await prisma_1.default.friend.findFirst({
            where: {
                OR: [
                    {
                        user_id1: userId,
        ...
        return prisma_1.default.$transaction([
            ...
            prisma_1.default.friend.delete({
                where: { id: friendship.id }
            }),
            // Bu iki kullanıcı arasındaki tüm arkadaşlık isteklerini sil
            prisma_1.default.friendship_request.deleteMany({
                where: {
                    OR: [
                        {
                            sender_id: userId,
     ...
     */
    static async getRequests(userId, status = 'pending') {
        return prisma_1.default.friendship_request.findMany({
            where: {
                receiver_id: userId,
                status
            },
     ...
     */
    static async checkPendingRequest(userId1, userId2) {
        const request = await prisma_1.default.friendship_request.findFirst({
            where: {
                OR: [
                    {
     ...
     */
    static async checkExistingRequest(userId1, userId2) {
        const request = await prisma_1.default.friendship_request.findFirst({
            where: {
                OR: [
                    {
    ...
    static async deleteRequest(requestId) {
        return prisma_1.default.friendship_request.delete({
            where: { id: requestId }
        });
    }
    /**
     ...
     */
    static async getSuggestedFriends(userId, limit = 5) {
        // 1. Mevcut arkadaşları bul
        const friends = await this.getFriends(userId);
        const friendIds = friends.map(friend => friend.id);
        // 2. Bekleyen arkadaşlık isteklerini bul
        const pendingRequests = await prisma_1.default.friendship_request.findMany({
            where: {
                OR: [
                    { sender_id: userId },
                    { receiver_id: userId }
        ...
        const pendingRequestUserIds = pendingRequests.flatMap(request => [request.sender_id, request.receiver_id]).filter(id => id !== userId);
